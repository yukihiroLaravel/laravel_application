<ul class="nav nav-tabs nav-justified mt-5 mb-2">
    ...
    <li class="nav-item nav-link {{ Request::is('users/'. $user->id) ? 'active' : '' }}"><a href="{{ route('user.timeline', $user->id) }}">アクティビティ<br>
            @php $totalActiveties = $countMovies + $countComments @endphp
            <div class="badge badge-secondary">{{ $totalActiveties ?? ''}}</div>
        </a></li>
